<div class="chat_body">
    <div class="chat_users">
        <div class="chat_users_display">
            <!--MENSAGEM DE UM UTILIZADOR-->
            <div class="chat_users_display_user">
                <div class="chat_users_display_user_img"><a><img src="Imagens/Icones/icons8-male-user-26.png"></a></div>
                <div class="chat_users_display_info">
                    <div class="chat_users_display_user_nome">Ruben do Tuning</div>
                    <div class="chat_users_display_user_mensagem">Queres ver o meu honda?</div>
                </div>
            </div>
            <!--FINAL-->
        </div>
        <div class="chat_users_settings">botao de escrever nova mensagem e settings</div>
    </div>
    <div class="chat_display">
        <div class="chat_display_user"></div>
        <div class="chat_display_text">
            <div class="chat_display_text_msg" ><input type="text" placeholder="Escreva a sua mensagem..."></div>
            <div class="chat_display_text_send"></div>
        </div>
    </div>
<div>
